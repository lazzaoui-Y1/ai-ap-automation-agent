...
class InvoiceLineItem(BaseModel):
    ...
    def validate_line_total(cls, v, values):
        ...
        if 'quantity' in values and 'unit_price' in values:
            expected = (Decimal(str(values['quantity'])) * values['unit_price']) - values.get('discount', Decimal("0"))
...
class VendorInfo(BaseModel):
    ...
    registration_number: Optional[str] = Field(None, description="رقم السجل التجاري / CR")
    ...
    mapped_vendor_code: Optional[str] = Field(None, description="كود المورد في نظام ERP")
...
class PaymentInfo(BaseModel):
    """معلومات الدفع"""
    payment_method: Optional[str] = Field(None, description="طريقة الدفع (نقدي، بنكي، آجل)")
    payment_terms: Optional[str] = Field(None, description="شروط الدفع (Net 30, Net 60)")
...
class Invoice(BaseModel):
    ...
    invoice_type: InvoiceType = Field(default=InvoiceType.STANDARD, description="نوع الفاتورة")
    invoice_date: date = Field(..., description="تاريخ الفاتورة")
    currency: Currency = Field(default=Currency.SAR, description="العملة")
    language_detected: Language = Field(default=Language.AR, description="اللغة المكتشفة")
    ...
    # Line Items - البنود
    # ═══════════════════════════════════════════════════
    line_items: List[InvoiceLineItem] = Field(..., min_length=1, description="بنود الفاتورة")
    
    # ═══════════════════════════════════════════════════
    # Financial Amounts
    # ═══════════════════════════════════════════════════
    subtotal: Decimal = Field(..., ge=0, description="المجموع الفرعي قبل الخصم والضريبة")
    total_discount: Decimal = Field(default=Decimal("0"), ge=0, description="إجمالي الخصم")
    ...
    # Tax Details
    # ═══════════════════════════════════════════════════
    tax_breakdown: Optional[List[TaxBreakdown]] = Field(None, description="تفصيل الضرائب")
    ...
    # Processing Metadata
    # ═══════════════════════════════════════════════════
    confidence_score: float = Field(default=0.0, ge=0, le=1, description="درجة الثقة في الاستخراج")
    extraction_timestamp: datetime = Field(default_factory=datetime.now, description="وقت الاستخراج")
    ...
    def validate_total(cls, v, values):
        ...
        if 'subtotal' in values and 'total_tax' in values and 'total_discount' in values:
            expected = values['subtotal'] - values['total_discount'] + values['total_tax']
            if abs(expected - v) > Decimal("0.01"):
...
class CustomerConfig(BaseModel):
    ...
    connector_type: str = Field(..., description="نوع الموصل: excel, csv, sap_sftp_xml, oracle_rest, webhook")
    
    # Connector-specific settings
    connector_config: Dict[str, Any] = Field(default_factory=dict, description="إعدادات خاصة بالموصل")
    ...
    languages: List[Language] = Field(default=[Language.AR, Language.EN], description="اللغات المدعومة")
    default_currency: Currency = Field(default=Currency.SAR, description="العملة الافتراضية")
